  ...
  try {
    ...
    if (type === "warden") {
      const result = await db.pool.query(
        "UPDATE complaint SET is_completed = NOT is_completed, assigned_at = CURRENT_TIMESTAMP WHERE complaint_id = $1 RETURNING *",
        [id]
      );
...
exports.getAllComplaintsByUser = async (req, res) => {
  const token = req.headers.authorization;
  console.log("Token in /complaints:", token);
  ...
  try {
    ...
    console.error("Error in getAllComplaintsByUser:", err.message);
  ...
  try {
    ...
    if (type == "warden") {
      const deleteComplaint = await db.pool.query(
        `delete from complaint where complaint_id = $1`,
        [id]
      );
